...
def taproot_construct(pubkey, scripts=[]):
    ...
    """
    tweak = tagged_hash("TapTweak", pubkey.get_bytes())
...
class TapLeaf:
    ...
    def construct_pk_hashlock_hash160_mod(self, key, data):
        pk_node = miniscript.pk(key.get_bytes())
        hash_node = miniscript.hash160_mod(data)
        v_c_pk_node = miniscript.v(miniscript.c(pk_node))
        self._set_miniscript(miniscript.and_v(v_c_pk_node, hash_node))
        self.desc = TapLeaf._desc_serializer('pk_hashlock', key.get_bytes().hex(), data.hex())
        return self
...
class miniscript:
    ...
    @staticmethod
    def hash160_mod(data):
        assert(len(data) == 20)
        script = lambda x: [OP_HASH160, data, OP_EQUAL]
