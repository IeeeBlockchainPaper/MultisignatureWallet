...
class ECPubKey():
    ...
    def reconstruct_h_for_ring_sign(self, sig, msg):
        assert(len(msg) == 32)
        assert(len(sig) == 64)
        assert(self.valid)
        h = int.from_bytes(sig[0:32], 'big')
        if h >= SECP256K1_FIELD_SIZE:
            return False
        s = int.from_bytes(sig[32:64], 'big')
        if s >= SECP256K1_ORDER:
            return False
        R = SECP256K1.affine(SECP256K1.mul([(SECP256K1_G,s), (self.p, (SECP256K1_ORDER - h))]))
        # print(R)
        R_x_bytes=R[0].to_bytes(32,'big')
        h = int.from_bytes(TaggedHash("BIP0340/challenge", R_x_bytes + self.get_bytes() + msg), 'big') % SECP256K1_ORDER
        h_bytes=h.to_bytes(32,byteorder='big')
        return h_bytes



def verify_ring_signature(pubkeys,signature,msg):
    assert(len(msg) == 32)

    #retrieve 1st challenge
    h1 = int.from_bytes(signature[0:32], 'big')
    if h1 >= SECP256K1_FIELD_SIZE:
        return False

    no_of_signers=len(pubkeys)

    #retrieve all responses
    s_list=list()
    for i in range(no_of_signers):
        s_list.append(int.from_bytes(signature[32*(i+1):32*(i+2)],'big'))
        # print(s_list[i])

    h=h1
    for i in range(no_of_signers):
        # print(h)
        R = SECP256K1.affine(SECP256K1.mul([(SECP256K1_G,s_list[i]), (pubkeys[i].p, (SECP256K1_ORDER - h))]))
        R_x_bytes=R[0].to_bytes(32,'big')
        # print(R)
        h = int.from_bytes(TaggedHash("BIP0340/challenge", R_x_bytes + pubkeys[i].get_bytes() + msg), 'big') % SECP256K1_ORDER
...
def generate_schnorr_nonce_and_point():
    """Generate a random valid BIP340 nonce.
